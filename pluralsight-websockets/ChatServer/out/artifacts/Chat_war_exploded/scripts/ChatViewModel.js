...
function ChatViewModel(chatClient) {
    ...
    var initialize = function (roomName) {
        chatClient.initialize(self.handler, roomName);
...
// public members
    ...
    self.join = function () {
        if (!self.joined() && self.userName() && self.userName().length > 0 && self.roomName() && self.roomName().length > 0) {
            self.joined(true);
            initialize(self.roomName());
        }
    };
